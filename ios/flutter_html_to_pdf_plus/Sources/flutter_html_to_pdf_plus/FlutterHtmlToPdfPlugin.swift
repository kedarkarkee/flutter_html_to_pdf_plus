...
public class FlutterHtmlToPdfPlugin: NSObject, FlutterPlugin {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(
            name: "flutter_html_to_pdf_plus", binaryMessenger: registrar.messenger())
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "convertHtmlToPdf":
            ...
            wkWebView = WKWebView.init(
                frame: CGRect(
                    origin: CGPoint(x: 0, y: 0), size: CGSize(width: width, height: height)))
            ...
            let formatter: UIPrintFormatter = UIMarkupTextPrintFormatter(
                markupText: htmlFileContent)

            urlObservation = wkWebView.observe(
                \.isLoading,
                changeHandler: { (webView, change) in
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
                        let convertedFileURL = PDFCreator.create(
                            printFormatter: formatter, width: width, height: height,
                            orientation: orientation as! String, margins: margins!)
                        let convertedFilePath = convertedFileURL.absoluteString
                            .replacingOccurrences(of: "file://", with: "")  // return generated pdf path
